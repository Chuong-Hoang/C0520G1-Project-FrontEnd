...
export class StatisticByRoomComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.totalAllUse(this.bookedRoomByRoom);
        ...
        for (let i = 0; i < arr.length; i++) {
            if (arr[i].roomType === 'Một lần') {
                this.barChartData.push({
                    data: [arr[i].effective, 0, 0, 0],
                    label: arr[i].roomName + ' sử dụng: ' + arr[i].totalUse + 'lần  '
                });
            }
            if (arr[i].roomType === 'Hàng ngày') {
                this.barChartData.push({
                    data: [0, arr[i].effective, 0, 0],
                    label: arr[i].roomName + ' sử dụng:' + arr[i].totalUse + 'lần  '
                });
            }
            if (arr[i].roomType === 'Hàng tuần') {
                this.barChartData.push({
                    data: [0, 0, arr[i].effective, 0],
                    label: arr[i].roomName + ' sử dụng: ' + arr[i].totalUse + 'lần  '
                });
            }
            if (arr[i].roomType === 'Hàng tháng') {
                this.barChartData.push({
                    data: [0, 0, 0, arr[i].effective],
                    label: arr[i].roomName + ' sử dụng: ' + arr[i].totalUse + 'lần  '
                });
            }
        }
    ...
    totalAllUse(arr: BookedRoom[]): void {
        // tslint:disable-next-line:prefer-for-of
        for (let i = 0; i < arr.length; i++) {
            this.totalALl += arr[i].totalUse;
        }
    }
}
